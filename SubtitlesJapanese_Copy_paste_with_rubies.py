...
class SubtitlePlayer:
    ...
    def parse_ruby_segments(self,text: str):
        segments = []
    ...
    def _render_subtitle(self, top_segments, bottom_segments):
        ...
        for segs in (top_segments, bottom_segments):
            for base, ruby in segs:
                if ruby:
                    rf = tkFont.Font(
                        family=self.subtitle_font.actual('family'),
                        size=int(self.subtitle_font.actual('size')*0.6),
                        weight="bold"
                    )
                    max_ruby_h = max(max_ruby_h, rf.metrics('linespace'))

        # total: ruby_above + line1 + line2 + ruby_below
        total_height = max_ruby_h + self.line_height*2 + max_ruby_h
        ...
        # Y positions
        y_ruby_top = max_ruby_h // 2
        y_base1   = max_ruby_h + self.line_height//2
        y_base2   = max_ruby_h + self.line_height + self.line_height//2
        y_ruby_bot = max_ruby_h + self.line_height*2 + max_ruby_h//2

        top_width = sum(self.subtitle_font.measure(b) for b,_ in top_segments)
        bottom_width = sum(self.subtitle_font.measure(b) for b,_ in bottom_segments)

        # starting X positions
        x_top = (self.max_width - top_width) / 2
        x_bot = (self.max_width - bottom_width) / 2

        # draw top line, if present
        cur_x = x_top
        if top_segments:
            for base, ruby in top_segments:
                w = self.subtitle_font.measure(base)
                center = cur_x + w/2
                if ruby:
                    rf = tkFont.Font(
                        family=self.subtitle_font.actual('family'),
                ...
                self.draw_outlined_text(
                    self.subtitle_canvas,
                    center, y_base1,
                    base, self.subtitle_font,
                    fill=self.subtitle_color, outline="black", thickness=3
                )
                cur_x += w
        ...
        new_y = self.sub_window_bottom_anchor - (max_ruby_h + self.line_height*2)
    ...
    def update_subtitle_display(self) -> None:
        ...
        raw = self.subtitles[index].content
        raw = self.CLEAN_PATTERN_1.sub('', raw)
        raw = regex.sub(r'(\p{Han}+)\(([^)]+)\)', r'\1«\2»', raw)
        raw = regex.sub(r'[（\(].*?[）\)]', '', raw)
        ...
        if not lines:
            ...
            top_segments = []
            bottom_segments = self.parse_ruby_segments(lines[0])
        else:
            top_segments = self.parse_ruby_segments(lines[0])
            bottom_segments = self.parse_ruby_segments(lines[1])

        joined = ''.join(base for base, _ in (top_segments + bottom_segments))
        if joined == self.last_subtitle_text and self.user_hidden:
            return
        if joined != self.last_subtitle_text:
            ...
            self.user_hidden = False
            self._render_subtitle(top_segments, bottom_segments)

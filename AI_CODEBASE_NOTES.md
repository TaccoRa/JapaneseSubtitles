...
- `SubtitlePlayer/app.py` creates `SubtitlePlayerApp`, loads `config.json`, builds the hidden Tk root, starts a subtitle-loading worker, then builds UI, renderer, and controller on the Tk thread.
...
2. `ConfigManager` reads `config.json`.
...
- `model/config_manager.py`: JSON-backed config wrapper with atomic save through a temporary file.
...
- `model/anki_client.py`: AnkiConnect API client, word normalization, translation, dictionary lookup, card field building, deck/card routing, media reuse, and stroke SVG sync.
...
- `config.json` stores both defaults and runtime state such as last file, window positions, and session time. This makes the repo look dirty during normal app usage. A better split is `config.default.json` in git and a local user config ignored by git.
...
2. Split runtime user config from repo defaults.

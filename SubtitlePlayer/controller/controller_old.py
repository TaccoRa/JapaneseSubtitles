...
import pyautogui
import bisect
from PIL import ImageOps, ImageGrab, ImageStat, Image
...
class SubtitleController:
    ...
    def __init__(self,
        ...
        self.settings.bind_open_srt                  (self.on_open_srt)
        ...
        if any(k in values for k in startup_value_keys):
            ...
            try:
                if "EXTRA_OFFSET" in values:
                    ...
                    self.settings._apply_offset_change(off, previous_value=old_off)
            except Exception:
                pass
            try:
                if "DEFAULT_SKIP" in values:
                    ...
                    self.settings._apply_skip_change(skip)
            except Exception:
                pass
            try:
    ...
    def _apply_time_text(self, text: str) -> float | None:
        text = text.strip()
    ...
    def on_set_to_return(self, text: str):
        ...
        secs = self._apply_time_text(text)
    ...
    def control_time_entry_return(self, event):
        ...
        text = self.settings.control_time_str.get()

        secs = self._apply_time_text(text)

        if secs is None:
            self.settings.control_time_str.set(
    ...
    def _ocr_auto_enabled(self) -> bool:
        ...
        if raw is None:
            return True
        return self.coerce_bool(raw, default=True)
    ...
    def _ocr_debug_enabled(self, override: dict | None = None) -> bool:
        if override and "OCR_DEBUG" in override:
            return self.coerce_bool(override.get("OCR_DEBUG"), default=True)
        return self.coerce_bool(self.config.get("OCR_DEBUG"), default=True)

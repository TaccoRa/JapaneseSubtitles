...
DEFAULT_SWITCH_RUNS = 20
DEFAULT_WARMUP_RUNS = 1
DEFAULT_SWITCH_STEPS = 50
DEFAULT_MAX_FILES_PER_ANIME = 3
DEFAULT_RESET_CACHE_EACH_RUN = False
...
def group_local_files_by_anime(subs_root: Path) -> Dict[str, List[Path]]:
    ...
    for srt_path in sorted(subs_root.rglob("*.srt")):
        try:
            ...
            anime_name = rel.parts[0] if len(rel.parts) >= 2 else "(root)"
        except Exception:
            anime_name = "(root)"
...
def make_target_for_path(manager: SubtitleManager, srt_path: Path) -> EpisodeTarget:
    season = None
    episode = None
    global_index = None
    try:
        season, episode, global_index = manager.extract_season_episode_global(srt_path.name)
    except Exception:
        pass
...
def install_timers(manager: SubtitleManager, renderer: SubtitleRenderer, controller: SubtitleController, stats, times):
    ...
    orig_change_episode_local = getattr(manager, "change_episode_local", None)
    ...
    def wrapped_change_episode_local(action, raw=None, target_season=None):
        t0 = time.perf_counter()
        result = orig_change_episode_local(action, raw, target_season)
        times["manager.change_episode_local"] += time.perf_counter() - t0
        stats["manager.change_episode_local_calls"] += 1
        return result
    ...
    if orig_change_episode_local is not None:
        manager.change_episode_local = wrapped_change_episode_local
...
def diff_metrics(after, before):
    ...
    out_stats = collections.Counter({k: v for k, v in stats.items() if v})

    out_times = {}
    after_times = after["times"]
    before_times = before["times"]
    keys = set(after_times) | set(before_times)
    for key in keys:
        out_times[key] = float(after_times.get(key, 0.0)) - float(before_times.get(key, 0.0))
    return out_stats, out_times
...
def build_local_environment(config_path: Path, srt_path: Path):
...
def run_episode_switch_sequence(
    ...
    if not targets:
        raise RuntimeError("No episode targets available for benchmark")

    sequence = targets[:]
    if len(sequence) < 2:
        sequence = sequence * 2

    # Warmup transitions to populate caches.
    ...
    before = snapshot_metrics(run_episode_switch_sequence.stats, run_episode_switch_sequence.times)
    ...
    for i in range(steps):
        ...
        try:
            ...
            # Keep the benchmark running even if one target fails.
            pass
    ...
    after = snapshot_metrics(run_episode_switch_sequence.stats, run_episode_switch_sequence.times)
...
run_episode_switch_sequence.stats = collections.Counter()
run_episode_switch_sequence.times = collections.defaultdict(float)


def print_breakdown(title: str, stats: collections.Counter, times: Dict[str, float]) -> None:
    print(f"\n=== {title} FUNCTION BREAKDOWN ===\n")
    print(f"{'Function':<34} {'Calls':>10} {'Total ms':>14} {'Avg ms':>14}")
    print("-" * 76)

    rows = [
        ("manager.change_episode_local", "manager.change_episode_local_calls"),
    ...
    for name, call_key in rows:
        ...
        print(f"{name:<34} {call_count:>10} {total_ms:>14.1f} {avg_ms:>14.3f}")
...
def summarize_group(results: List[Dict[str, object]]) -> Dict[str, float]:
    ...
    total_switches = 0
    for r in results:
        total_create_text += int(r.get("stats", collections.Counter()).get("canvas.create_text_calls", 0))
        total_switches += int(r.get("stats", collections.Counter()).get("manager.change_episode_local_calls", 0))
        total_switches += int(r.get("stats", collections.Counter()).get("manager.change_episode_remote_calls", 0))
    if total_switches <= 0:
...
def make_local_target_list(manager: SubtitleManager, selected_files: List[Path]) -> List[EpisodeTarget]:
    out = []
    for p in selected_files:
        out.append(make_target_for_path(manager, p))
    return out
...
def benchmark_local(
    ...
    warmup: int,
    steps: int,
    max_files_per_anime: int,
    reset_cache_each_run: bool,
) -> None:
    subs_root = ROOT / "subs"
    if not subs_root.exists():
        print(f"Local benchmark skipped: {subs_root} not found")
        return

    groups = group_local_files_by_anime(subs_root)
    if not groups:
        print("Local benchmark skipped: no .srt files found under subs/")
        return

    random.seed(DEFAULT_RANDOM_SEED)

    print("\n" + "=" * 90)
    print("LOCAL EPISODE SWITCH BENCHMARK")
    print("=" * 90)
    print(f"Runs per file-group: {runs}")
    ...
    for anime_name, files in groups.items():
        selected_files = choose_evenly_spaced(files, max_files_per_anime)
        if len(selected_files) < 2:
            print(f"\n[{anime_name}] skipped (needs at least 2 episode files for switching)")
            continue
        ...
        root, config, manager, overlay, renderer, controller = build_local_environment(config_path, selected_files[0])
        stats = collections.Counter()
        times = collections.defaultdict(float)
        install_timers(manager, renderer, controller, stats, times)
        run_episode_switch_sequence.stats = stats
        run_episode_switch_sequence.times = times

        targets = make_local_target_list(manager, selected_files)
        file_results: List[Dict[str, object]] = []

        try:
            # Prime the first switch so the loop starts from a real loaded state.
            controller.settings.episode_var.set(targets[1].label)
            try:
                switch_to_target(manager, controller, targets[1])
                controller._after_episode_change()
            except Exception:
                pass
            ...
            for _ in range(runs):
                ...
                result = run_episode_switch_sequence(
            ...
            group_summary = summarize_group(file_results)
            print("\n--- FILE / GROUP SUMMARY ---")
            print(f"Average switch time: {group_summary['avg_ms']:.2f} ms")
            print(f"Std dev of run avgs: {group_summary['stddev_ms']:.2f} ms")
            print(f"Best observed switch: {group_summary['best_ms']:.2f} ms")
            print(f"Worst observed switch: {group_summary['worst_ms']:.2f} ms")
            print(f"Average create_text:  {group_summary['avg_create_text']:.0f}")
            hit_rate = (
            ...
            print_breakdown(f"LOCAL / {anime_name}", stats, times)
    ...
    if overall_results:
        ...
        print(f"Worst observed:      {max(r['min_ms'] for r in overall_results):.2f} ms")
        print("=" * 90)
...
def benchmark_remote(
    ...
    if remote_env is None:
        print("\nRemote benchmark skipped: no remote configuration found (REMOTE_FLAG/LAST_GITHUB_URL not set).")
        return
    ...
    if not remote_map:
        print("\nRemote benchmark skipped: no remote_episode_map_global entries available.")
        try:
            root.destroy()
        except Exception:
            pass
    ...
    for _g, rec in remote_map.items():
        ...
        group_name = parts[1] if len(parts) >= 2 and parts[0] == "subtitles" else (parts[0] if parts else "remote")
    ...
    try:
        for group_name, items in sorted(grouped.items(), key=lambda kv: kv[0].casefold()):
            ...
            if len(selected) > 6:
                idxs = [0, len(selected) // 4, len(selected) // 2, (3 * len(selected)) // 4, len(selected) - 1]
                selected = [selected[i] for i in dict.fromkeys(idxs)]

            targets = [make_target_for_remote_record(rec) for rec in selected]
            if len(targets) < 2:
            ...
            controller.settings.episode_var.set(targets[0].label)
            try:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Benchmark episode switching for local and remote subtitle paths.")
    ...
    parser.add_argument("--runs", type=int, default=DEFAULT_SWITCH_RUNS)
    ...
    parser.add_argument("--max-files-per-anime", type=int, default=DEFAULT_MAX_FILES_PER_ANIME)
    parser.add_argument("--reset-cache-each-run", action="store_true", default=DEFAULT_RESET_CACHE_EACH_RUN)
    ...
    if args.mode in ("local", "both"):
        benchmark_local(
            ...
            warmup=args.warmup,
            steps=args.steps,
            max_files_per_anime=args.max_files_per_anime,

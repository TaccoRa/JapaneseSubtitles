...
def extract_kanji_from_any_html(html_text: str) -> set[str]:
    text = strip_html_to_text(html_text or "")
    text = INLINE_READING_RE.sub("", text)
    return {ch for ch in text if is_kanji_char(ch)}


def extract_note_kanji(note_fields: dict[str, Any]) -> set[str]:
    out: set[str] = set()
    front = (note_fields or {}).get("Front")
    if not isinstance(front, dict):
        return out

    value = front.get("value", "")
    if not value:
        return out

    chars = extract_kanji_from_html(value)
    if chars:
        out.update(chars)
    else:
        # Fallback when the kanji wrapper is applied in the card template
        out.update(extract_kanji_from_any_html(value))
    return out
...
def main() -> int:
    ...
    print(f"Notes containing kanji: {notes_with_kanji}")
